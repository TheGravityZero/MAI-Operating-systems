...
int send_message(zmq::socket_t& socket, const std::string& message_string) 
{
...
std::string recieve_message(zmq::socket_t& socket) 
{
    ...
    bool ok = false;
    try { ok = socket.recv(&message); } 
    catch(...) { ok = false; }
    std::string recieved_message(static_cast<char*>(message.data()), message.size());
    if(recieved_message.empty() || !ok) { return "Error: Node is not available";}
    return recieved_message;
}

std::string get_port_name(int port) { return "tcp://127.0.0.1:" + std::to_string(port); }
